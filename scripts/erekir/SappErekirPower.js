...
const matterCollapser = extend(ConsumeGenerator, "matter-collapser", {
    ...
    setStats() {
        ...
        for (let i = 0; i < sappItemsStats.items.length; i++) {
            this.stats.add(sappItemsStats.costs[i], StatValues.content(sappItemsStats.items[i]));
        }
        
        for (let i = 0; i < sappLiquidsStats.liquids.length; i++) {
            this.stats.add(sappLiquidsStats.temperatures[i], StatValues.content(sappLiquidsStats.liquids[i]));
        }
    },
    ...
    load() {
        ...
        this.sliderRegion2 = Core.atlas.find(this.name + "-slider2");
    },
...
matterCollapser.buildType = () => extend(ConsumeGenerator.ConsumeGeneratorBuild, matterCollapser, {
    ...
    draw() {
        ...
        let item = this.getItem();
        let liquid = this.getLiquid();
        let mC = matterCollapser;
        let rotateSpeed1 = 0.1;
        let rotateSpeed2 = 0.1;

        if (this.items.total() > 0 && this.liquids.currentAmount() > 0) {
            rotateSpeed1 = (item.cost * liquid.temperature) * 1.2;
            rotateSpeed2 = (item.cost * liquid.temperature) * 1.5;
        }

        Draw.rect(mC.region, this.x, this.y);
        Drawf.spinSprite(mC.sliderRegion1, this.x, this.y, -Time.time * rotateSpeed1);
        Drawf.spinSprite(mC.sliderRegion2, this.x, this.y, Time.time * rotateSpeed2);
   },
