...
const execute = extend(StatusEffect, "execute", {
	reactive: true,
    /*init(){
        this.affinity(paganism, (unit, result, time) => {
                    unit.damagePierce(1800);
                    Fx.shockwave.at(unit.x + Mathf.range(unit.bounds() / 2), unit.y + Mathf.range(unit.bounds() / 2));
                    result.set(execute, Math.min(time + result.time, 200));
                });
    },*/
});
